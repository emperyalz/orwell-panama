...
export const socialArchive=internalAction({args:{},handler:async(ctx):Promise<{inserted:number;matched:number}>=>{
 ...
 const items=posts.flatMap(post=>{const author=people.filter(p=>normalize(p.name)===normalize(post.entityName));const matches=author.length===1?author:people.filter(p=>normalize(post.caption||'').includes(normalize(p.name)));return matches.length===1?[{politicianId:matches[0]._id,kind:'social' as const,sourceUrl:post.postUrl,sourceName:post.entityName,title:post.caption?Array.from(post.caption).slice(0,180).join(''):'Publicación',summary:post.caption?Array.from(post.caption).slice(0,700).join(''):undefined,platform:post.platform==='twitter'?'x_twitter':post.platform,publishedAt:post.postedAt,...(post.thumbnailStorageId&&post.thumbnailUrl?.includes('.convex.cloud')?{imageUrl:post.thumbnailUrl}:{})}]:[];});let inserted=0;for(let i=0;i<items.length;i+=50)inserted+=await ctx.runMutation(internal.activity.ingest,{items:items.slice(i,i+50)});await ctx.runMutation(internal.activity.logRun,{kind:'social-archive',inserted,errors:[]});return {inserted,matched:items.length};
}});
