...
export default async function Multiplayer({searchParams}:{searchParams:Promise<{politician?:string;tab?:string;session?:string}>}){
 ...
 return <WorkshopLoader politicianId={id} name={person?.name||id} initialTab={tab} versionSession={query.session} people={people.map(p=>({id:p.externalId,name:p.name,party:p.party}))}><ProfilePage params={Promise.resolve({id})} initialPanel={tab}/></WorkshopLoader>;
}
