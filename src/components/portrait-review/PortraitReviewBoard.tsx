...
type Person={id:string;category:'missing'|'replace';name:string;role:string;province:string;headshot:string|null;hasHeadshot:boolean;references:StartingReference[];pilot:string|null;master:string|null};
 ...
 return <article className="portrait-review-person" id={person.id}>
  ...
  <div className="portrait-review-images">
   ...
   <div className={`portrait-review-gallery${dragging?' is-dragging':''}`} onDragEnter={e=>{if(canEdit){e.preventDefault();setDragging(true);}}} onDragOver={e=>{if(canEdit)e.preventDefault();}} onDragLeave={e=>{if(!e.currentTarget.contains(e.relatedTarget as Node))setDragging(false);}} onDrop={e=>{if(!canEdit)return;e.preventDefault();setDragging(false);void addFiles(e.dataTransfer.files);}}>
    <div className="portrait-review-gallery-toolbar"><span>{person.master?'1 cuerpo completo · 1 recorte · ':person.pilot?'1 borrador · ':''}{references.length} {references.length===1?'imagen':'imágenes'} de referencia</span><div className="portrait-review-actions">{canEdit&&<><input ref={input} type="file" accept="image/jpeg,image/png,image/webp,image/avif" multiple hidden onChange={e=>{if(e.target.files)void addFiles(e.target.files);}}/><button type="button" className="portrait-review-upload" onClick={()=>input.current?.click()} disabled={busy}><Plus size={16}/> Añadir fotos</button></>}<button type="button" aria-label={`Desplazar las referencias de ${person.name} a la izquierda`} onClick={()=>scroll(-1)} disabled={!position.left}><ChevronLeft size={17}/></button><button type="button" aria-label={`Desplazar las referencias de ${person.name} a la derecha`} onClick={()=>scroll(1)} disabled={!position.right}><ChevronRight size={17}/></button></div></div>
    <div className="portrait-review-scroller" ref={scroller} onScroll={updatePosition}>{person.master&&<figure className="portrait-review-reference portrait-review-master"><a className="portrait-review-image" href={person.master} target="_blank" rel="noreferrer" aria-label={`Abrir retrato de cuerpo completo de ${person.name}`}><img src={person.master} alt={`Retrato de cuerpo completo de ${person.name}`} loading="lazy"/></a><figcaption><strong>Cuerpo completo</strong><a href={person.master} target="_blank" rel="noreferrer">Ver completo ↗</a></figcaption></figure>}{person.pilot&&<figure className="portrait-review-reference portrait-review-pilot"><a className="portrait-review-image" href={person.pilot} target="_blank" rel="noreferrer" aria-label={`Abrir recorte de ${person.name}`}><img src={person.pilot} alt={`Recorte para el sitio de ${person.name}`} loading="lazy"/></a><figcaption><strong>{person.master?'Recorte · 800 × 1000':'Borrador IA'}</strong><a href={person.pilot} target="_blank" rel="noreferrer">Ver completo ↗</a></figcaption></figure>}{references.map((reference,index)=><figure className="portrait-review-reference" key={reference._id??`initial-${index}`}>
