...
const navigation=[{href:'/',label:'Personas'},{href:'/oficinas',label:'Cargos'},{href:'/partidos',label:'Partidos'},{href:'/destacados',label:'Actividad'},{href:'/proyectos',label:'Proyectos'},{href:'/metodologia',label:'Fuentes'}];
...
export function ProfileHeader({publicPath}:{publicPath?:string}){
 ...
 const active=path.startsWith('/proyectos')?'/proyectos':path.startsWith('/partidos')?'/partidos':path.startsWith('/oficinas')?'/oficinas':/^\/(destacados|noticias)/.test(path)?'/destacados':path.startsWith('/metodologia')?'/metodologia':/^\/(politician|comparar)/.test(path)||path==='/'?'/':null;
