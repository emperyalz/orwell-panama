...
type Props={politicianId:string;name:string;initialTab:string;versionSession?:string;children:ReactNode};
export default function WorkshopClient({politicianId,name,initialTab,versionSession,children}:Props){
 ...
 const base=`orwell-panama-${politicianId}-${tab}`;const sessionId=versionSession&&new RegExp(`^${base}-v[0-9]+$`).test(versionSession)?versionSession:base;
 useEffect(()=>{const onTab=(event:Event)=>{const next=(event as CustomEvent<{id:string}>).detail.id;const url=new URL(window.location.href);url.searchParams.set('politician',politicianId);url.searchParams.set('tab',next);url.searchParams.delete('session');history.replaceState(null,'',url);setTab(next);};window.addEventListener('orwell:profile-tab',onTab);return()=>window.removeEventListener('orwell:profile-tab',onTab);},[politicianId]);
 ...
 if(!identity)return <div className="workshop-login"><form onSubmit={login}><img src="/icons/branding/orwell-black.svg" alt="ORWELL" width={161} height={32}/><h1>Multiplayer</h1><p>Entra con tu cuenta de equipo para editar y anotar los perfiles.</p><label>Usuario<input name="username" autoComplete="username" value={username} onChange={e=>setUsername(e.target.value)} placeholder="Eric o Jose" required/></label><label>Contraseña<input type="password" name="password" autoComplete="current-password" value={password} onChange={e=>setPassword(e.target.value)} required/></label>{error&&<p role="alert">{error}</p>}<button type="submit">Entrar</button><a href={`/politician/${politicianId}`}>Ver perfil público</a></form></div>;
 return <MultiplayerProvider api={api} config={{convexDeployment:'fleet-vole-527',storageKeyPrefix:'orwell-panama',brand:{name:'ORWELL Panamá',primaryColor:'#59348b'}}}><WorkshopTools key={sessionId} sessionId={sessionId} identity={identity} name={name} onSwitch={()=>{localStorage.removeItem(loginKey);setIdentity(null);}}/><div className="workshop-content"><div onClickCapture={e=>{const link=(e.target as HTMLElement).closest('a');const href=link?.getAttribute('href');if(href&&/^\/politician\/[^/?#]+$/.test(href)&&!e.ctrlKey&&!e.metaKey){e.preventDefault();e.stopPropagation();window.location.href=`/multiplayer?politician=${encodeURIComponent(href.split('/')[2])}`;}}}><ProfileHeader/>{children}</div></div></MultiplayerProvider>;
}
